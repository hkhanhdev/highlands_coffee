...
TABLE_TYPE = "lg"
TABLE_IDENTIFIER = ("raw_test", "transaction_details") #logical table identifier in Iceberg catalog
TABLE_LOCATION = "s3://highlands-coffee-warehouse/raw_test/transaction_details" #physical location in MinIO bucket
# Create output schema
R_SCHEMA = Schema(
    NestedField(1, "id", LongType(), required=False),
    NestedField(2, "order_id", LongType(), required=False),
    NestedField(3, "product_id", LongType(), required=False),
    NestedField(4, "product_details_id", LongType(), required=False),
    NestedField(5, "quantity", LongType(), required=False),
    NestedField(6, "subtotal", LongType(), required=False),
    NestedField(7, "discount_amount", DoubleType(), required=False),
    NestedField(8, "subtotal_after_discount", DoubleType(), required=False),
)

E_SCHEMA = Schema(
    NestedField(1, "id", IntegerType(), required=False),
    NestedField(2, "order_id", IntegerType(), required=False),
    NestedField(3, "product_id", IntegerType(), required=False),
    NestedField(4, "product_details_id", IntegerType(), required=False),
...
def main():
    ...
    # catalog.drop_table(("raw_test", "transaction_details"))
    # table = catalog.load_table(("raw_test", "transaction_details"))
    # print(table.schema())

    print("Transaction Details module executed")
