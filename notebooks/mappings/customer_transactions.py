...
TABLE_TYPE = "lg"
TABLE_IDENTIFIER = ("raw_test", "customer_transactions") #logical table identifier in Iceberg catalog
TABLE_LOCATION = "s3://highlands-coffee-warehouse/raw_test/customer_transactions" #physical location in MinIO bucket
# Create output schema
R_SCHEMA = Schema(
    NestedField(1, "id", LongType(), required=False), 
    NestedField(2, "order_date", StringType(), required=False), 
    NestedField(3, "cus_id", LongType(), required=False), 
    NestedField(4, "coupon_id", DoubleType(), required=False), 
    NestedField(5, "store_id", LongType(), required=False), 
    NestedField(6, "payment_method_id", LongType(), required=False), 
    NestedField(7, "status", StringType(), required=False),
)

E_SCHEMA = Schema(
    NestedField(1, "id", IntegerType(), required=False), 
    NestedField(2, "order_date", DateType(), required=False), 
    NestedField(3, "cus_id", IntegerType(), required=False), 
    NestedField(4, "coupon_id", IntegerType(), required=False), 
    NestedField(5, "store_id", IntegerType(), required=False), 
    NestedField(6, "payment_method_id", IntegerType(), required=False), 
    NestedField(7, "status", StringType(), required=False),
)
...
def main():
    ...
    # catalog.drop_table(("raw_test", "customer_transactions"))
    # table = catalog.load_table(("raw_test", "customer_transactions"))
    # print(table.schema())

    print("Customer Transactions module executed")
