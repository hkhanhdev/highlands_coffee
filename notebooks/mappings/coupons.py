...
TABLE = "coupons"
TABLE_TYPE = "sm"
TABLE_IDENTIFIER = ("raw_test", "coupons") #logical table identifier in Iceberg catalog
TABLE_LOCATION = "s3://highlands-coffee-warehouse/raw_test/coupons" #physical location in MinIO bucket
# Create output schema
R_SCHEMA = Schema(
    NestedField(1, "id", LongType(), required=False),
    NestedField(2, "coupon_name", StringType(), required=False),
    NestedField(3, "effective_date", StringType(), required=False),
    NestedField(4, "discount_type", StringType(), required=False),
    NestedField(5, "discount_value", LongType(), required=False),
    NestedField(6, "product_usage_limit", DoubleType(), required=False),
    NestedField(7, "usage_limit", DoubleType(), required=False),
    NestedField(8, "effective_member_id", LongType(), required=False),
    NestedField(9, "expiry_date", StringType(), required=False),
)
...
def r_2_e(source_path: str,source_table:str,source_date:str):
    ...
    # Reading input data using Dask
    # df_result = getRawData(day=source_date, table_name=TABLE, engine="dask", table_type=TABLE_TYPE)
    # if isinstance(df_result, tuple): #Handle error if any
    #     return df_result
    # df = df_result
