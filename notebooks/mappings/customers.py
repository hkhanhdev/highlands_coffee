...
from pyiceberg.types import NestedField, LongType, FloatType, StringType, IntegerType,DateType,DecimalType,DoubleType,TimestampType
...
TABLE = "customers"
TABLE_TYPE = "lg"
TABLE_IDENTIFIER = ("raw_test", "customers") #logical table identifier in Iceberg catalog
TABLE_LOCATION = "s3://highlands-coffee-warehouse/raw_test/customers" #physical location in MinIO bucket
# Create output schema
R_SCHEMA = Schema(
            NestedField(1, "id", LongType(), required=False),
            NestedField(2, "cus_name", StringType(), required=False),
            NestedField(3, "current_mem_id", LongType(), required=False),
            NestedField(4, "prev_mem_id", LongType(), required=False),
            NestedField(5, "last_updated_at", DateType(), required=False),
            NestedField(6, "registration_date", DateType(), required=False),
        )

E_SCHEMA = Schema(  
            NestedField(1, "id", IntegerType(), required=False),
            NestedField(2, "cus_name", StringType(), required=False),
            NestedField(3, "current_mem_id", IntegerType(), required=False),
            NestedField(4, "prev_mem_id", IntegerType(), required=False),
            NestedField(5, "last_updated_at", DateType(), required=False),
            NestedField(6, "registration_date", DateType(), required=False),
        )
...
def main():
    ...
    # catalog.drop_table(("raw_test", "customers"))
    # table = catalog.load_table(("raw_test", "customers"))
    # print(table.schema())

    print("Customers module executed")
